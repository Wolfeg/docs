...
Возвращает строку с языком, заданным в [настройках текущего пользователя](../../settings/index.md#user-settings).

#### Поддерживаемые виды чартов {#get-lang-charts}

Все.

#### Аргументы {#get-lang-args}

Нет.

#### Возвращаемый результат {#get-lang-result}

Строка, соответствующая языку, заданному в настройках пользователя: `"ru"` или `"en"`.

#### Пример {#get-lang-example}
...
- Результат
  ...
  Значение переменной `lang` после выполнения вкладки [Prepare](./tabs.md#prepare) при текущем значении настройки языка пользователя — `ru`:
