...
* Added support for {{ ZK }} in clusters running {{ KF }} 3.6 to 3.9, inclusive.
...
Now you can [change the availability zone](./operations/host-migration.md) for {{ mkf-name }} clusters.

## Q4 2023 {#q4-2023}

New {{ KF }} 3.5.1 is available in the `PRODUCTION` environment. For more information about the updates, see [this {{ KF }} 3.5.0 guide](https://archive.apache.org/dist/kafka/3.5.0/RELEASE_NOTES.html) and [this {{ KF }} 3.5.1 guide](https://archive.apache.org/dist/kafka/3.5.1/RELEASE_NOTES.html).

## Q2 2023 {#q2-2023}

New {{ KF }} 3.4 is available in the `PRODUCTION` environment. For more information about the updates, see [this {{ KF }} guide](https://archive.apache.org/dist/kafka/3.4.0/RELEASE_NOTES.html).

## Q1 2023 {#q1-2023}

New {{ KF }} 3.3 is available in the `PRODUCTION` environment.
...
* New {{ KF }} 3.2 is available in the `PRODUCTION` environment.

## Q2 2022 {#q2-2022}

* A new connector is now available: [S3 Sink](concepts/connectors.md#s3-sink).
* New {{ KF }} 3.1 is available in the `PRESTABLE` and `PRODUCTION` environments.
